...
namespace UnityEngine.Rendering.Universal
{
    ...
    public abstract partial class ScriptableRenderPass
    {
        ...
        /// 设置attachmentIndex指定的颜色附着的数据存储方式
        /// </summary>
        ...
        /// <summary>
        /// 配置绘制目标的颜色附着rt,并设置它的格式
        /// </summary>
        /// <param name="colorAttachment">渲染目标颜色附着的引用</param>
        /// <param name="format">rt的格式</param>
        /// <param name="width"></param>
        /// <param name="height"></param>
        /// <param name="sampleCount">是否是MSAA</param>
        /// <param name="depth">是否是depthOnly</param>
        ...
        /// 在渲染一个相机前被renderer调用，配置rt和它们的清理状态，创建临时的rt纹理,如果renderpass没有重写这个方法，这个renderpass就渲染到被激活的相机rt上
        /// </summary>
        ...
        /// If a render pass doesn't override this method, this render pass renders to the active Camera's render target.
        /// You should never call CommandBuffer.SetRenderTarget. Instead call <c>ConfigureTarget</c> and <c>ConfigureClear</c>.
        /// 在执行renderer pass之前被renderer调用，重写这个方法，需要配置rt和它们的清理状态，以及临时的rt纹理,如果renderpass没有重写这个方法，这个renderpass就渲染到被激活的相机rt上
        /// </summary>
        /// <param name="cmd">CommandBuffer to enqueue rendering commands. This will be executed by the pipeline.</param>
        ...
        /// 在渲染完相机之后调用，可以用这个调用来清理被这个renderer创建的资源
        /// </summary>
        ...
        /// 根据物体的排序，支持的shaderpass类型(lightMode)和渲染数据创建绘制设置
        /// </summary>
