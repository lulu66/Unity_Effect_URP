...
namespace UnityEngine.Rendering.Universal.Internal
{
	...
	public class PostProcessPass : ScriptableRenderPass
	{
		...
		// 是一种编码高动态范围颜色的技术,当rt格式不支持hdr时，就需要将高动态范围颜色(>1)编码进低位数的格式的rt中
		bool m_UseRGBM;
		...
		// 判断当前pass是否是管线的最后一个后处理Pass,是的话返回true
		bool m_IsFinalPass;
		...
		// 判断当前pass之后是否还有一个final post process pass，如果有的话，film grain和dithering效果就要在final pass中去设置，没有就在当前pass中设置
		bool m_HasFinalPass;

		// Some Android devices do not support sRGB backbuffer
		// We need to do the conversion manually on those
		// 部分机型不支持sRGB backbuffer，需要手动转换格式
		...
		// true:内容拷贝到屏幕 false:color frontbuffer
		...
		public PostProcessPass(RenderPassEvent evt, PostProcessData data, Material blitMaterial)
		{
			...
			// 一次性加载内置的效果所需的shader资源，并批量创建材质球
			m_Materials = new MaterialLibrary(data);

			m_BlitMaterial = blitMaterial;

			// 根据目标平台是否支持特定的图形格式，来决定默认的hdr格式
			...
			// graphicsDeviceVendor:当前图形设备的制造商/供应商,可能是 "NVIDIA", "AMD", "Intel", "ARM", "Apple", "Qualcomm"
			// ToLowerInvariant:将字符串转换为小写，忽略语言文化
			// smaa效果相关
		...
		public void Setup(in RenderTextureDescriptor baseDescriptor, in RenderTargetHandle source, bool resolveToScreen, in RenderTargetHandle depth, in RenderTargetHandle internalLut, bool hasFinalPass, bool enableSRGBConversion)
		{
			...
			// MSAA和Resolve（解析）之间的关系，不同的解析策略
			// true:让图形驱动/硬件在将图像呈现到屏幕之前，自动执行 MSAA 解析
			m_UseSwapBuffer = true;
		}

		// 这个用于Universal2D，暂时忽略
		public void Setup(in RenderTextureDescriptor baseDescriptor, in RenderTargetHandle source, RenderTargetHandle destination, in RenderTargetHandle depth, in RenderTargetHandle internalLut, bool hasFinalPass, bool enableSRGBConversion)
		{
			...
			// false:由Unity渲染管线在代码中显式地执行MSAA解析
		...
		void Render(CommandBuffer cmd, ref RenderingData renderingData)
		{
			...
			// 如果开启了双缓冲，并且需要双缓冲的渲染效果存在，就要关闭MSAA
			...
			// 使用双缓冲系统的话，source为back buffer, destination为front buffer
			...
			RenderTargetIdentifier GetDestination()
			{
				...
				{
					// 如果不使用双缓冲系统，则需要创建临时rt来作为destination
					if (destination == -1)
					{
			...
			// Lens Flare 最后一个需要swap buffer的效果，不需要再交换buffer了
			if (useLensFlare)
			{
				bool usePanini;
			...
			using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
			{
				...
				// 如果有final pass则gain和dither在final pass进行，否则在post process pass执行
				...
				// destination:如果用了双缓冲系统则是系统的front buffer,如果没有使用，则是屏幕；
				// m_Destination : 就是屏幕
				...
				// 使用了双缓冲系统，且需要绘制于屏幕上的话，渲染目标就是离线纹理或者屏幕；如果不需要绘制于屏幕上，渲染目标就是front buffer或者屏幕
				if (m_UseSwapBuffer)
				{
...
#endif
				{
					...
					// 如果绘制目标是屏幕且没使用双缓冲系统 或者 需要将内容绘制于屏幕上且使用了双缓冲系统，都要设置相机的绘制区域
					// cameraData.pixelRect:相机渲染区域在屏幕像素空间中的矩形
		...
		void RenderFinalPass(CommandBuffer cmd, ref RenderingData renderingData)
		{
			...
			if (cameraData.imageScalingMode != ImageScalingMode.None)
			{
				...
				//---------------------GAUSSF START-----------------------
				bool isGaussfEnabled = ((cameraData.imageScalingMode == ImageScalingMode.Upscaling) && (cameraData.upscalingFilter == ImageUpscalingFilter.GAUSSF));
				//---------------------GAUSSF END-------------------------
				...
				// 是否需要初始化配置（如果开启了fxaa或者fsr则需要）
				bool isSetupRequired = (isFxaaEnabled || isFsrEnabled || isGaussfEnabled);
				...
				if (isSetupRequired)
				{
					...
					if(isGaussfEnabled)
					{
						int colorSpaceIdx = (renderingData.cameraData.gaussfQualityLevel == 0) ? 1 : 0;
						m_Materials.scalingSetup.EnableKeyword(ShaderKeywordStrings.GaussF_ColorSpace[colorSpaceIdx]);
					}
					// 绘制在_ScalingSetupTexture这个RT上
					cmd.GetTemporaryRT(ShaderConstants._ScalingSetupTexture, tempRtDesc, FilterMode.Point);
					isScalingSetupUsed = true;

					// 拷贝source到scallingup texture上
					Blit(cmd, m_Source, ShaderConstants._ScalingSetupTexture, m_Materials.scalingSetup);

					// 然后将scallingup texture作为source
					...
					case ImageScalingMode.Upscaling:
						{
							...
							switch (cameraData.upscalingFilter)
							{
								...
								case ImageUpscalingFilter.GAUSSF:
									{
										m_Materials.gaussfUpscaler.shaderKeywords = null;

										var upscaleRtDesc = cameraData.cameraTargetDescriptor;
										upscaleRtDesc.msaaSamples = 1;
										upscaleRtDesc.depthBufferBits = 0;
										upscaleRtDesc.enableRandomWrite = renderingData.cameraData.gaussfQualityLevel == 1;
										upscaleRtDesc.width = cameraData.pixelWidth;
										upscaleRtDesc.height = cameraData.pixelHeight;
										upscaleRtDesc.graphicsFormat = renderingData.cameraData.gaussfQualityLevel == 1 ? UnityEngine.Experimental.Rendering.GraphicsFormat.R8G8B8A8_UNorm : UniversalRenderPipeline.MakeUnormRenderTextureGraphicsFormat();

										cmd.GetTemporaryRT(ShaderConstants._UpscaledTexture, upscaleRtDesc, FilterMode.Point);
										switch (renderingData.cameraData.gaussfQualityLevel)
										{
											case 0: // Level 3(GF1.5.1)
												{
													var gaussfInputSize = new Vector2(cameraData.cameraTargetDescriptor.width, cameraData.cameraTargetDescriptor.height);
													var gaussfOutputSize = new Vector2(cameraData.pixelWidth, cameraData.pixelHeight);

													GAUSSFUtils.SetUpscalingConstants(cmd, gaussfInputSize, gaussfInputSize, gaussfOutputSize, cameraData.gaussfSharpness);

													Blit(cmd, sourceRtId, ShaderConstants._UpscaledTexture, m_Materials.gaussfUpscaler, 2);

													material.EnableKeyword(ShaderKeywordStrings.GaussF_ColorSpace[1]);
												}
												break;
											case 1: // Level 0(GF2.0 Neural network)
												{
													ComputeShader gaussfCS = m_Data.shaders.gaussfUpscalerCS;
													gaussfCS.SetInt("width", cameraData.pixelWidth / 2);
													gaussfCS.SetInt("height", cameraData.pixelHeight / 2);

													cmd.SetComputeTextureParam(gaussfCS, 0, "Source", sourceRtId);
													cmd.SetComputeTextureParam(gaussfCS, 0, "Target", ShaderConstants._UpscaledTexture);

													cmd.DispatchCompute(gaussfCS, 0, cameraData.pixelWidth / 32 + 1, cameraData.pixelHeight / 32 + 1, 1);

													material.EnableKeyword(ShaderKeywordStrings.GaussF_ColorSpace[0]);

												}
												break;
											case 2: // Level 4(GF2.5 Ultra Performance)
												{
													var gaussfInputSize = new Vector2(cameraData.cameraTargetDescriptor.width, cameraData.cameraTargetDescriptor.height);
													GAUSSFUtils.SetUpscalingConstants(cmd, gaussfInputSize);

													Blit(cmd, sourceRtId, ShaderConstants._UpscaledTexture, m_Materials.gaussfUpscaler, 3);
													material.EnableKeyword(ShaderKeywordStrings.GaussF_ColorSpace[0]);
		...
		class MaterialLibrary
		{
			...
			// GaussF 
			public readonly Material gaussfUpscaler;
			...
			public MaterialLibrary(PostProcessData data)
			{
				...
				// GaussF
				gaussfUpscaler = Load(data.shaders.gaussfUpscalerPS);
			...
			internal void Cleanup()
			{
				...
				// GaussF
				CoreUtils.Destroy(gaussfUpscaler);
