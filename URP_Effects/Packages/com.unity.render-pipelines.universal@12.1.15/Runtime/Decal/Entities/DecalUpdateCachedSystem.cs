...
namespace UnityEngine.Rendering.Universal
{
    ...
    internal class DecalUpdateCachedSystem
    {
        ...
        {
            // 绕X轴逆时针旋转90度,变换之后的效果是：Y轴变成Z轴；Z轴变成-Y轴
            ...
            /// <summary>
            /// decal的位置、旋转、缩放发生变化时，重新计算decal的投影矩阵和边界球体
            /// </summary>
            ...
            public void Execute(int index, TransformAccess transform)
            {
                ...
                // 判断decal的位置、旋转和缩放是否发生变化，若发生变化，则修改decal的位置
                ...
                // 计算两种不同的缩放模式下的localtoworld矩阵
                float4x4 localToWorld;
                if (scaleModes[index] == DecalScaleMode.InheritFromHierarchy)
                {
                    localToWorld = transform.localToWorldMatrix;
                    // 将贴花从常规3D物体空间变换至投影至贴花的特殊空间(因为贴花总是沿着表面法线的方向投影,让贴花的向上方向从Y轴变成Z轴，更适合表面投影)                 
            ...
            /// <summary>
            /// 球体包围盒的计算，单位cube -> 从贴花空间转换到世界空间 -> 计算center和radius
            /// </summary>
            /// <param name="decalToWorld"></param>
            /// <returns></returns>
