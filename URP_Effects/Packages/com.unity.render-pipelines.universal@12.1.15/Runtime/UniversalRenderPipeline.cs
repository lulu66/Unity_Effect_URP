    ...
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        static void RenderSingleCamera(ScriptableRenderContext context, CameraData cameraData, bool anyPostProcessingEnabled)
        {
            ...
            using (new ProfilingScope(cmdScope, sampler)) // Enqueues a "BeginSample" command into the CommandBuffer cmd
            {
                ...
                // 根据相机数据、裁剪结果和是否开启后处理，获取渲染数据renderingData
            ...
            if (cameraStack != null)
            {
                ...
                for (int i = 0; i < cameraStack.Count; ++i)
                {
                    ...
                    if (currCamera.isActiveAndEnabled)
                    {
                        ...
                        // 判断当前相机的renderer是否和baseCamera的renderer的类型一致，只有一致的renderer的camera可以被stack
...
#pragma warning restore 0618
        ...
        /// 从baseCamera中获取cameraData数据，作为stack中所有camera的设置
        /// </summary>
